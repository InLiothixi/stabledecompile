...
{
	if (!DefinitionLoadXML(StringToSexyString(theFileName), &gReanimatorDefMap, theDefinition))
		return false;
...
bool Reanimation::DrawTrack(Graphics* g, int theTrackIndex, int theRenderGroup, TodTriangleGroup* theTriangleGroup)
{
	...
	{
		aMatrix.LoadIdentity();
		int aWidth = aTransform.mFont->StringWidth(StringToSexyString(aTransform.mText));
	...
	{
		TodDrawStringMatrix(g, aTransform.mFont, aMatrix, StringToSexyString(aTransform.mText), aColor);
		if (mEnableExtraAdditiveDraw)
		{
			int aOldMode = g->GetDrawMode();  // 备份绘制模式
			g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
			TodDrawStringMatrix(g, aTransform.mFont, aMatrix, StringToSexyString(aTransform.mText), aExtraAdditiveColor);
...
void Reanimation::UpdateAttacherTrack(int theTrackIndex)
{
	...
	if (aAttacherInfo.mTrackName.size() != 0)  // 如果定义了附属动画的动作轨道
	{
		...
		aAttachReanim->GetFramesForLayer(SexyStringToString(aAttacherInfo.mTrackName).c_str(), aAnimFrameStart, aAnimFrameCount);
		if (aAttachReanim->mFrameStart != aAnimFrameStart || aAttachReanim->mFrameCount != aAnimFrameCount)  // if (!aAttachReanim->IsAnimPlaying(……))
		{
			...
			aAttachReanim->SetFramesForLayer(SexyStringToString(aAttacherInfo.mTrackName).c_str());  // 播放指定轨道上的动作
		}

		if (aAttachReanim->mAnimRate == 12.0f && aAttacherInfo.mTrackName.compare(_S("anim_walk")) == 0 && aAttachReanim->TrackExists("_ground"))

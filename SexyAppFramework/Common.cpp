...
{
#ifdef _USE_WIDE_STRING
	return std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>>{}.from_bytes(theString);
#else 
...
SexyString Sexy::GetFileDir(const SexyString& thePath, bool withSlash)
{
	int aLastSlash = max((int)thePath.rfind(_S('\\')), (int)thePath.rfind(_S('/')));

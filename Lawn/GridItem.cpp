...
void GridItem::DrawGridItem(Graphics* g)
{
    ...
    {
    case GridItemType::GRIDITEM_GRAVESTONE:         DrawGraveStone(g);                              return;
    case GridItemType::GRIDITEM_CRATER:             DrawCrater(g);                                  return;
    case GridItemType::GRIDITEM_LADDER:             DrawLadder(g);                                  return;
    ...
    case GridItemType::GRIDITEM_BRAIN:              g->DrawImageF(IMAGE_BRAIN, mPosX, mPosY);       return;
    case GridItemType::GRIDITEM_SCARY_POT:          DrawScaryPot(g);                                return;
...
void GridItem::ClosePortal()
{
    ...
    if (aPortalReanim)
    {
        aPortalReanim->PlayReanim("anim_dissapear", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 12.0f); // Originally: "anim_disappear" (It have a typo on the reanim)
    }
...
void GridItem::UpdateScaryPot()
{
    ...
    {
        if (aPlant->mSeedType == SeedType::SEED_PLANTERN && !aPlant->NotOnGround() && aPlant->mBurnedCounter == -1)
        {

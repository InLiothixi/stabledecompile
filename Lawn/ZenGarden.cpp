...
void ZenGarden::PlantUpdateProduction(Plant* thePlant)
{
    ...
    if (thePlant->mLaunchCounter <= 100)
    {
        int aFlashCountdown = TodAnimateCurve(100, 0, thePlant->mLaunchCounter, 0, 100, TodCurves::CURVE_LINEAR);
        thePlant->mEatenFlashCountdown = max(thePlant->mEatenFlashCountdown, aFlashCountdown);
    }
